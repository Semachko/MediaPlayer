...
ImageConverter::ImageConverter(const Codec& codec) {
    ...
    converter = sws_getContext(codec.context->width,
...
ImageConverter::~ImageConverter() { sws_freeContext(converter); }

Frame ImageConverter::convert(Frame input) {
    sws_scale(converter, input->data, input->linesize, 0, output->height, output->data, output->linesize);
    return output;
}
