...
Filters::Filters(const Codec& codec, VideoParameters* params_) :
    ...
    args = "video_size=" + std::to_string(codec.parameters->width) + "x" + std::to_string(codec.parameters->height) +
           ":pix_fmt=" + std::to_string(codec.parameters->format) + ":time_base=" + std::to_string(codec.timeBase.num) +
           "/" + std::to_string(codec.timeBase.den);
    update_filters();
...
void Filters::update_filters() {
    ...
    std::string temp = "eq=brightness=" + std::to_string(params->brightness) +
