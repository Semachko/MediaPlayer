...
VideoPreview::~VideoPreview() {
    QMetaObject::invokeMethod(
        params->video->previewSink,
        [videoSink = params->video->previewSink] { videoSink->setVideoFrame(QVideoFrame()); },
        Qt::QueuedConnection);
    delete converter;
    delete decoder;
    delete codec;
...
void VideoPreview::update_preview(qreal seconds) {
    ...
    for (;;) {
        ...
        while (!queue.empty()) {
            ...
            QImage image(output_frame->data[0],
                         codec->context->width,
            ...
            QMetaObject::invokeMethod(
                params->video->previewSink,
                [videoSink = params->video->previewSink, frame = std::move(QVideoFrame(image))] {
