...
AudioOutputer::AudioOutputer(Clock* clock_, Codec& codec_, SampleFormat format_, MediaParameters* params_,
                             QAudioSink* sink) :
        QIODevice(),
    ...
    bytes_per_second = out_format.layout.nb_channels * out_format.sample_rate *
                       av_get_bytes_per_sample((AVSampleFormat)out_format.format);
}

AudioOutputer::~AudioOutputer() { clear(); }

void AudioOutputer::push_frame(Frame frame) {
    qint64 frame_size =
        frame->nb_samples * frame->ch_layout.nb_channels * av_get_bytes_per_sample((AVSampleFormat)frame->format);
...
qint64 AudioOutputer::readData(char* data, qint64 maxlen) {
    ...
    if (!processed_frame)
        return 0;
    qint64 dataSize =
        processed_frame->nb_samples * processed_frame->ch_layout.nb_channels * out_format.bytes_per_sample;
...
qint64 AudioOutputer::writeData(const char* data, qint64 maxSize) { return -1; }
