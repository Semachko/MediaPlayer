...
SampleConverter::SampleConverter(AVCodecContext* input, SampleFormat output) :
        output_format(output) {
    // av_log_set_level(AV_LOG_DEBUG);
    converter_context = swr_alloc();

    int ret = swr_alloc_set_opts2(&converter_context,
                                  &output_format.layout,
                                  (AVSampleFormat)output_format.format,
...
SampleConverter::~SampleConverter() { swr_free(&converter_context); }
...
Frame SampleConverter::convert(Frame input) {
    ...
    output->nb_samples = av_rescale_rnd(swr_get_delay(converter_context, input->sample_rate) + input->nb_samples,
                                        output->sample_rate,
                                        input->sample_rate,
                                        AV_ROUND_UP);
    ...
    int nb_samples = swr_convert(
        converter_context, output->data, output->nb_samples, (const uint8_t**)input->data, input->nb_samples);
    if (nb_samples < 0) {
...
Frame SampleConverter::flush() {
    ...
    output->nb_samples = av_rescale_rnd(delay, output->sample_rate, output->sample_rate, AV_ROUND_UP);
    ...
    int ret = swr_convert(converter_context, output->data, output->nb_samples, nullptr, 0);
...
void SampleConverter::clear() { swr_convert(converter_context, nullptr, 0, nullptr, 0); }
