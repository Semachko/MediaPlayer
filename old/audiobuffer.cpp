...
qint64 AudioBuffer::sync_copy_to_buffer(char* data, qint64 len) {
    ...
    //     (processed_frame->ch_layout.nb_channels * format.bytes_per_sample);
    //     qint64 dataSize = processed_frame->nb_samples *
    //     processed_frame->ch_layout.nb_channels * format.bytes_per_sample -
    //     new_data_pos; if (dataSize <= 0)
    //         continue;
    //     const char* data = reinterpret_cast<const
    //     char*>(processed_frame->data[0]) + new_data_pos;
...
void AudioBuffer::clear() { buffer.clear(); }
